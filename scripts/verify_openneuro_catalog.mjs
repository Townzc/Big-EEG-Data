...
const index = JSON.parse(fs.readFileSync(new URL("../data/openneuro-fmri-index.json", import.meta.url), "utf8"));
const expected = new Map(index.datasets.map((dataset) => [dataset.accession, dataset.name]));
const manuallyCurated = new Map(
  [...catalog.matchAll(/openNeuro\(\{\s*accession: "(ds\d+)",\s*name: "([^"]+)"/g)]
    .map((match) => [match[1], match[2]]),
);
for (const [accession, name] of manuallyCurated) if (!expected.has(accession)) expected.set(accession, name);
...
for (let offset = 0; offset < accessions.length; offset += 20) {
  ...
  if (!payload.data) throw new Error(JSON.stringify(payload.errors ?? "OpenNeuro API returned no data"));
...
console.log(`Verified ${records.length} OpenNeuro accessions against API v${apiVersion}; ${missing.length} missing, ${renamed.length} snapshot/top-level display names differ.`);
for (const record of renamed) console.log(`${record.id}\tSNAPSHOT: ${expected.get(record.id)}\tTOP-LEVEL: ${record.name}`);
if (missing.length) {
