...
## 当前更新：2026-09-13 / UCDDB 微伏确认

**当前 97 个疾病条目：68 个有统一格式产物，已下载尚未执行统一格式处理的为 0，29 个未下载。** UCDDB 已按负责人确认的 μV 重跑 25 份 / 173.360556 h；保留 EDF 增益/偏移后统一为 μV/100，全部独立校验通过。

疾病标准分支合计 **189,944 份 / 45,487.667001 h**；各库可追踪身份 28,938，按有证据共享关系合并后 25,494，含对照且不保证匿名库间全球唯一。32,376 份其他分支的单位仍为推断/待核。

格式为 200 Hz、float32 `[channel,time]` NPZ + JSON/CSV。UCDDB 的 25 份源记录均有实际 ADC 超头部声明量程的问题，增益沿用头部但缺独立校准证据；量程、分期对齐及其他 QC 均保留。EEG-0053 沿用已确认微伏的 μV/100 产物。

[UCDDB 换算详情](PREPROCESSING_UCDDB_UV_20260913.zh-CN.md) · [六项及全部疾病明细](PREPROCESSING_SIX_20260913.zh-CN.md) · [97 项状态](DISEASE97_STATUS_20260913.zh-CN.md)

以下日期小节为历史快照；原 99 项与旧 14 项的跨分类口径见 [范围对应表](DISEASE_SCOPE_RECONCILIATION_20260913.zh-CN.md)。
