...
**按网站全部 97 个疾病条目核对：62 个已处理，6 个已下载未统一预处理，29 个未下载。** 待处理六项为 HMC、ADSZ、Albrecht2019、Singh2020、Singh2021、UCDDB，均在旧 99 项清单之外。[当前 97 项状态与证据](DISEASE97_STATUS_20260913.zh-CN.md)。

分类补充：原 99 项中，当前归疾病的 **76 项 = 61 已处理 + 15 未取得**；其他分类 **22 项 = 15 已处理 + 7 未取得**；BEED 排除 1 项。原本地 14 项包含 9 个疾病目标，不能整体减掉。完整网站目录有 **97 个疾病条目**，其中 21 个在旧 99 项清单之外；已处理疾病目标为 62。

旧 99 项已逐项对账：**76 项有时域处理产物 + 22 项未取得 + 1 项 BEED 排除 = 99**。原“14 个已下载”随后已完成 13 项时域处理及 BEED 排除。当前疾病类 62 = 76 − 15 个已归其他分类的目标 + 旧 99 之外的 EEG-0060。两种 62 不能混用；单位/标签待核另列。[完整对应关系与原 14 项去向](DISEASE_SCOPE_RECONCILIATION_20260913.zh-CN.md)。

**EEG-0053 已按你确认的 μV 从原始 MAT 重跑并全量验证**：121 人（61 ADHD / 60 Control），19 通道、200 Hz、真实 `/100` 缩放，4.7013597222 h；98/7/16 train/validation/test。旧 native 隔离分支不再计入当前数据。

**疾病类现状：62 个目标、189,256 份完整产物、44,078.886232 h**；各库已识别人数合计 28,451，已知共享身份合并后 25,007 个可追踪身份（含对照，不宣称跨所有匿名库的全球唯一人数）。659 份缺可靠人物映射；犬类、VitalDB 和已重分类目标单列。全部 255,471 份本次选用数组的全值/哈希/格式校验通过。

已修复 MODMA 三通道旧 int32 溢出（7 文件 / 1,703,972 个值），以 int64 无损重建并验证；物理校准仍待确认。已修正 TBI 视觉分支的 raw/processed 分类和联合训练的 75 组身份划分冲突、九组完全相同信号跨 split；AES/UPenn 的负责人单位/参考确认已同步本地。32,142 份疾病产物仍有单位推断/待核，标签、参考与幅值问题逐项保留。**[完整中文报告与 62 项明细](DISEASE_AUDIT_20260913.zh-CN.md)** · [CSV](public/disease-preprocessing-summary-20260913.csv) · [汇总 JSON](data/disease-preprocessing-audit-20260913.json)。

以下日期小节是历史快照。2026-09-10 对 EEG-0053 的“单位未知/native 隔离”描述已由本节取代。VitalDB 仍归意识与状态 / 麻醉，EEG-0077 归 cognitive，BEED 从当前目录排除。

## 当前更新：2026-09-10

VitalDB（**EEG-0609**，“意识与状态 / Anesthesia”）已完成预处理：**5,565 个病例 / 5,344 名患者，16,785 个连续段，18,097.513249 信号小时**，采用 200 Hz、60 Hz 陷波和已核实的 μV/100。全部 6,388 个源病例均已核对；病例 4552 因包时钟歧义完整保留原始数据包，并单列隔离。

**EEG-0053 的 121 人已完成 19 通道 / 200 Hz 预处理并通过全量校验**（61 ADHD / 60 Control，约 4.70 h）。通道编号 DOCX 已从镜像补回；单位/增益尚未核实，因此保留原数值尺度，存放于隔离分支。官方 S3 仍为 403，信号与文档来自已记录 commit 和校验值的公开镜像，坐标 CED 未取得。处理参数、输出位置、例外和完整校验数字见 [本次预处理记录](PREPROCESSING_UPDATE_20260910.zh-CN.md)。
...
当前仍为 **568 EEG / 784 fMRI**，八大 EEG 类别为 `4 / 97 / 65 / 142 / 59 / 135 / 43 / 23`。重点下载清单 145 行：81 有完成证据，76 取得独立 raw，62 个完成精确时长审计。目录、CSV/JSON 和 Excel 的上次同步详情见 [目录更新记录](CATALOG_UPDATE_20260910.zh-CN.md)。以下较早日期小节为历史快照。
