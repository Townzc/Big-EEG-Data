...
test('six acquired disease sources retain physical-unit and person-identity gates', async () => {
  ...
  assert.equal(batch.standardProcessedEntries, 68);
  assert.equal(batch.downloadedCalibrationBlockedEntries, 0);
  ...
  assert.equal(ucddb.outputUnit, 'uV/100');
  assert.equal(ucddb.unitStatus, 'documented');
  assert.equal(ucddb.unitConfirmationSource, 'project_owner');
  assert.equal(ucddb.edfGainUvPerCount, 1 / 4095);
  assert.equal(ucddb.edfOffsetUv, 2048 / 4095);
  assert.equal(ucddb.unitReviewOutputs, 0);
  assert.equal(ucddb.outputs, 25);
  assert.equal(ucddb.verifiedSubjectEntries, 25);
  assert.equal(audit.datasets[ucddb.id].outputs, 25, 'only the current UCDDB microvolt derivatives enter totals');
  assert.equal(audit.ucddb_unit_confirmation.unit_confirmation.authority, 'project_owner');
  assert.equal(audit.ucddb_unit_confirmation.qc_flag_counts.source_exceeds_declared_edf_digital_range, 25);
  assert.equal(audit.ucddb_unit_confirmation.qc_flag_counts.sleep_stage_time_alignment_unverified, 25);
  assert.match(audit.ucddb_unit_confirmation.calibration_limitation, /not independent correctness/);
  ...
  const standard = batch.entries;
  assert.equal(batch.standardOutputsAdded, 688);
...
test('current EEG metrics distinguish source scope and fully audited human disease outputs', async () => {
  ...
  try {
    ...
    assert.equal(preprocessing.auditedTargets, 68);
    assert.equal(preprocessing.outputs, 189_944);
    assert.equal(preprocessing.subjectEntries, 28_938);
    assert.equal(preprocessing.traceableIdentities, 25_494);
    assert.equal(preprocessing.outputsWithoutReliableSubject, 731);
    assert.ok(approx(preprocessing.signalHours, 45_487.66700138692));
    assert.equal(preprocessing.eventRows, 3_023_264);
    assert.equal(preprocessing.derivativeBytes, 1_740_379_377_252);
    assert.equal(preprocessing.unitReviewTargets, 20);
    assert.equal(preprocessing.unitConfirmedTargets, 48);
    assert.equal(preprocessing.unitReviewOutputs, 32_376);
    assert.equal(preprocessing.unitConfirmedOutputs, 156_159);
